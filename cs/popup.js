...
(function() {
  var $, bindOracle, chatterText, clickDinnerLink, clickHours, createBusDataRequest, displayItems, fadeButtonText, findUpdatedPosts, insertBusInfo, iteration, listDinners, ls, mainLoop, newsLimit, optionsText, tipsText, updateAffiliationNews, updateBus, updateCantinas, updateCoffee, updateHours, updateMeetings, updateServant,
  ...
  bindOracle = function() {
    return $('#oracle #question').keypress(function(e) {
      var question;
      if (e.which === 13) {
        question = $('#oracle #question').val();
        if (question !== '') {
          return Oracle.get(question, function(answer) {
            return $('#oracle #answer').text(answer);
          });
        } else {
          return $('#oracle #answer').text(Oracle.greet());
        }
      }
    });
  };
